...
CHECKS=(
  ...
  "check-echo.sh:Uses echo in logic script"
...
echo "Testing check scripts exit codes..."
...
for check_info in "${CHECKS[@]}"; do
  ...
  if bash ".config/$script" > /dev/null 2>&1; then
    echo "  [OK] $script exits 0 when passing"
  else
    echo "  [FAIL] $script exits non-zero on valid code"
    FAILED=1
  fi
done
...
if [[ $FAILED -eq 1 ]]; then
  echo ""
  echo "Some check scripts have incorrect exit codes"
  exit 1
fi

echo ""
echo "All check scripts have correct exit codes"
